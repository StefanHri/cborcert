...
impl Saving for Out<'_> {
    fn save(&self) -> Result<(), CborCertError> {
        match self {
            ...
            Out::OutVer(cert_verified_data) => cert_verified_data.save(),
        }
    }
}
...
impl Saving for OutVer<'_> {
    fn save(&self) -> Result<(), CborCertError> {
        for file in self.out_files {
            println!("file.format {:?}", file.format);
            match &file.format {
                FileFormat::C => {
                    return Err(CborCertError::VerCannotBeSavedInCFile);
                }
                FileFormat::TOML => {
                    //todo remove unwrap
                    println!(
                        "Saving the certificate content to {}",
                        file.full_name.to_string()
                    );
                    let all = toml::to_string(&self.cert).unwrap();
                    fs::write(file.full_name.to_string(), &all)?;
                }
                FileFormat::DER => {
                    return Err(CborCertError::VerCannotBeSavedInDerFile);
                }
            }
        }
        Ok(())
